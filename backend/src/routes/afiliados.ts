...
export default async function afiliadoAdminRoutes(app: FastifyInstance) {
  ...
  app.put('/retiros/:retiroId', auth, async (request, reply) => {
    ...
    const soporte = comprobanteAGuardar(comprobanteBase64, retiro.comprobanteBase64);
  ...
  app.delete('/:id', auth, async (request, reply) => {
    ...
    await prisma.$transaction([
      prisma.usuario.deleteMany({ where: { afiliadoId: id } }),
      prisma.afiliado.delete({ where: { id } }),
    ]);

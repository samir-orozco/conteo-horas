...
async function main() {
  const antes = await prisma.diaEsperado.count();
  console.log(`Filas antes: ${antes}`);

  const t0 = process.hrtime.bigint();
  const { colaboradores, dias } = await backfillTodos({
    info: (m: string) => console.log(m),
    error: (e: unknown, m?: string) => console.error(m, e),
  });
  const segs = Number(process.hrtime.bigint() - t0) / 1e9;

  const despues = await prisma.diaEsperado.count();
  ...
  const porColaborador = await prisma.diaEsperado.groupBy({
    by: ['colaboradorId'],
    _count: { _all: true },
    _min: { fecha: true },
    _max: { fecha: true },
  ...
  const nombres = new Map(
    (await prisma.colaborador.findMany({ select: { id: true, nombre: true, apellido: true } }))

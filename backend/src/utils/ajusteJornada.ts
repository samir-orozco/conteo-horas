...
function instanteDe(fecha: Date, hhmm: string): Date {
  return new Date(fecha.getTime() + minutosDe(hhmm) * MS_MIN);
}

export function ajustarAJornada(
  entrada: Date,
  salida: Date,
  dia: DiaParaAjuste,
): { entrada: Date; salida: Date; ajustada: boolean } {
  const sinCambio = { entrada, salida, ajustada: false };
  if (!dia.programado || dia.toleranciaSalidaMin <= 0) return sinCambio;
  if (!dia.horaEntrada || !dia.horaSalida) return sinCambio;
  ...
  if (salidaProg <= entradaProg) salidaProg = new Date(salidaProg.getTime() + UN_DIA_MS);
  ...
  if (deMas > 0 && deMas <= tolMs) nuevaSalida = salidaProg;
  ...
  if (dia.ajustaEntrada) {
    ...
    if (deAntes > 0 && deAntes <= tolMs) nuevaEntrada = entradaProg;
  }
  ...
  if (nuevaSalida.getTime() < nuevaEntrada.getTime()) return sinCambio;

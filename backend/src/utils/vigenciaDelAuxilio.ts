...
export const VIGENCIA_INVALIDA = 'INVALIDA' as const;

export type VigenciaNormalizada = {
  vigenteDesde: Date;
  valor: number;
  tope: number;
};
...
function medianocheBogota(texto: unknown): Date | null {
  ...
  if (fecha.getUTCMonth() !== Number(mes) - 1 || fecha.getUTCDate() !== Number(dia)) return null;
  return fecha;
}
...
export function normalizarVigencia(cuerpo: Record<string, unknown>): VigenciaNormalizada | typeof VIGENCIA_INVALIDA {
  const vigenteDesde = medianocheBogota(cuerpo.vigenteDesde);
  if (!vigenteDesde) return VIGENCIA_INVALIDA;

  const valor = numero(cuerpo.valor);
  const tope = numero(cuerpo.tope);
  if (valor === null || tope === null) return VIGENCIA_INVALIDA;

  // Un auxilio en cero sí es válido: un gobierno podría no decretarlo. Negativo no.
  if (valor < 0) return VIGENCIA_INVALIDA;
  // Un tope en cero dejaría a todo el mundo sin derecho.
  if (tope <= 0) return VIGENCIA_INVALIDA;
  ...
  if (tope < valor) return VIGENCIA_INVALIDA;

...
async function festivoRoutes(app) {
    ...
    app.get('/', auth, async (request) => {
        ...
        return prisma_1.prisma.diaFestivo.findMany({ where, orderBy: { fecha: 'asc' } });
    });
    // Agrega un día no laboral propio de la empresa (ej. día cívico)
    app.post('/', auth, async (request, reply) => {
        const data = request.body;
        const festivo = await prisma_1.prisma.diaFestivo.create({
    ...
    app.delete('/:id', auth, async (request, reply) => {
        ...
        const festivo = await prisma_1.prisma.diaFestivo.findFirst({ where: { id, empresaId: request.empresaId } });
        if (!festivo)
            return reply.status(404).send({ error: 'Solo puedes eliminar festivos propios de tu empresa' });
        return prisma_1.prisma.diaFestivo.delete({ where: { id } });
    });
    ...
    app.post('/generar/:anio', auth, async (request) => {
        ...
        for (const f of festivos) {
            const existente = await prisma_1.prisma.diaFestivo.findFirst({ where: { empresaId: null, fecha: f.fecha } });
            if (!existente) {
                await prisma_1.prisma.diaFestivo.create({ data: { fecha: f.fecha, nombre: f.nombre } });
                creados++;
            }
        }

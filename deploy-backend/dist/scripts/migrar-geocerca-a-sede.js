...
async function main() {
    const empresas = await prisma_1.prisma.empresa.findMany({ select: { id: true, nombre: true } });
    console.log(`Empresas: ${empresas.length}\n`);
    let creadas = 0, saltadas = 0, asignaciones = 0;
    for (const empresa of empresas) {
        const yaTiene = await prisma_1.prisma.sede.count({ where: { empresaId: empresa.id } });
        if (yaTiene > 0) {
        ...
        if (colaboradores.length > 0) {
            await prisma_1.prisma.colaboradorSede.createMany({

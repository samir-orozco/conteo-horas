...
exports.VIGENCIA_REFORMA = exports.PRORROGAS_ANTES_DE_UN_ANIO = exports.ANIOS_TOPE_APRENDIZAJE = exports.ANIOS_TOPE_FIJO = exports.DIAS_PREAVISO = void 0;
exports.estadoDelContrato = estadoDelContrato;
exports.DIAS_PREAVISO = 30;
exports.ANIOS_TOPE_FIJO = 4;
exports.ANIOS_TOPE_APRENDIZAJE = 3;
exports.PRORROGAS_ANTES_DE_UN_ANIO = 4;
...
exports.VIGENCIA_REFORMA = new Date(Date.UTC(2025, 5, 25, 5, 0, 0));
const DIA_MS = 24 * 60 * 60 * 1000;
const dias = (desde, hasta) => Math.round((hasta.getTime() - desde.getTime()) / DIA_MS);
const sumarDias = (f, n) => new Date(f.getTime() + n * DIA_MS);
...
const sumarAnios = (f, n) => new Date(Date.UTC(f.getUTCFullYear() + n, f.getUTCMonth(), f.getUTCDate(), 5, 0, 0));
function estadoDelContrato(contrato, prorrogas, hoy) {
    ...
    const fechaLimitePreaviso = finVigente ? sumarDias(finVigente, -exports.DIAS_PREAVISO) : null;
    ...
    let topeMaximo = null;
    let arranqueTope = null;
    if (tipo === 'FIJO') {
        arranqueTope = fechaInicio < exports.VIGENCIA_REFORMA ? exports.VIGENCIA_REFORMA : fechaInicio;
        topeMaximo = sumarAnios(arranqueTope, exports.ANIOS_TOPE_FIJO);
    }
    else if (tipo === 'APRENDIZAJE') {
        arranqueTope = fechaInicio;
        topeMaximo = sumarAnios(arranqueTope, exports.ANIOS_TOPE_APRENDIZAJE);
    }
    ...
    const proximaProrrogaMinimaUnAno = tipo === 'FIJO' && duracionOriginal > 0 && duracionOriginal < 365
        && enOrden.length >= exports.PRORROGAS_ANTES_DE_UN_ANIO;
    // Etapa del aprendiz
    let etapa = null;
    ...
    if (vigente && fechaLimitePreaviso) {
        if (preavisoVencido)
            alertas.push({ tipo: 'PREAVISO_VENCIDO', dias: diasParaVencer });
        else if (diasParaPreaviso <= exports.DIAS_PREAVISO)
            alertas.push({ tipo: 'PREAVISO_PROXIMO', dias: diasParaPreaviso });
    }
    if (proximaProrrogaMinimaUnAno)
        alertas.push({ tipo: 'PRORROGA_MINIMO_UN_ANO', dias: null });
    if (yaSuperaElTope)
        alertas.push({ tipo: 'SUPERA_TOPE', dias: null });
    else if (vigente && seVuelveIndefinidoEl) {
        const d = dias(hoy, seVuelveIndefinidoEl);
        if (d >= 0 && d <= exports.DIAS_PREAVISO)
            alertas.push({ tipo: 'SE_VUELVE_INDEFINIDO', dias: d });
    }
    if (etapa === 'LECTIVA' && contrato.fechaInicioPractica) {
        const d = dias(hoy, contrato.fechaInicioPractica);
        if (d >= 0 && d <= exports.DIAS_PREAVISO)
            alertas.push({ tipo: 'CAMBIO_ETAPA_APRENDIZ', dias: d });
    }
    return {

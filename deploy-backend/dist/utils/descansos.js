...
exports.claveDeVentana = exports.MAX_MARCACIONES_POR_JORNADA = exports.MAX_DESCANSOS_POR_FRANJA = void 0;
exports.horaValida = horaValida;
exports.minutosDesdeLaEntrada = minutosDesdeLaEntrada;
exports.leerVentana = leerVentana;
exports.leerDescansos = leerDescansos;
exports.escribirDescansos = escribirDescansos;
exports.ventanasEnOrden = ventanasEnOrden;
exports.unirIntervalos = unirIntervalos;
exports.minutosDeLaUnion = minutosDeLaUnion;
exports.minutosEnLasVentanas = minutosEnLasVentanas;
exports.minutosDescansoADescontar = minutosDescansoADescontar;
exports.descuentoDeCadaDescanso = descuentoDeCadaDescanso;
exports.descansoQueToca = descansoQueToca;
exports.ventanasDeLasSalidas = ventanasDeLasSalidas;
exports.ventanasTomadas = ventanasTomadas;
exports.regresoEsperadoDelDescanso = regresoEsperadoDelDescanso;
exports.emparejarSalidasDeDescanso = emparejarSalidasDeDescanso;
exports.completarVentanasDeDescanso = completarVentanasDeDescanso;
...
function minutosDesdeLaEntrada(horaEntrada, hhmm) {
    return ((0, tardanzas_1.minutosDe)(hhmm) - (0, tardanzas_1.minutosDe)(horaEntrada) + 1440) % 1440;
}
...
const claveDeVentana = (v) => `${v.inicio}-${v.fin}`;
exports.claveDeVentana = claveDeVentana;
...
function leerDescansos(valor) {
    let elementos;
    if (typeof valor === 'string') {
        if (valor.length > LARGO_MAXIMO_DEL_TEXTO)
            return [];
    ...
    for (const elemento of elementos) {
        ...
        if (!v || vistas.has((0, exports.claveDeVentana)(v)))
            continue;
        vistas.add((0, exports.claveDeVentana)(v));
        lista.push(v);
    }
    return lista;
}
...
function escribirDescansos(ventanas) {
...
function ventanasEnOrden(horaEntrada, ventanas) {
    const entrada = horaValida(horaEntrada);
    const posicion = (v) => (entrada === null ? (0, tardanzas_1.minutosDe)(v.inicio) : minutosDesdeLaEntrada(entrada, v.inicio));
...
function unirIntervalos(intervalos) {
    ...
    for (const [a, b] of orden) {
        ...
        if (ultimo && a <= ultimo[1])
            ultimo[1] = Math.max(ultimo[1], b);
        else
            unidos.push([a, b]);
    }
    return unidos;
...
function minutosDeLaUnion(horaEntrada, ventanas) {
    const entrada = horaValida(horaEntrada);
    return unirIntervalos(ventanas.map(v => {
        const desde = entrada === null ? (0, tardanzas_1.minutosDe)(v.inicio) : minutosDesdeLaEntrada(entrada, v.inicio);
        return [desde, desde + (0, tardanzas_1.duracionFranjaMin)(v.inicio, v.fin)];
...
function minutosEnLasVentanas(tramos, fecha, ventanas) {
    const intervalos = unirIntervalos(ventanas.flatMap(v => {
        const { inicio, fin } = (0, almuerzo_1.instantesDe)({ fecha, inicio: v.inicio, fin: v.fin });
        return [[inicio, fin], [inicio + UN_DIA_MS, fin + UN_DIA_MS]];
    }));
    const cruza = (ini, fin) => tramos.reduce((s, t) => s + (0, almuerzo_1.solape)(t.entrada.getTime(), t.salida.getTime(), ini, fin), 0);
...
// almuerzo.
function minutosDescansoADescontar(marcas, dia) {
    return Math.round(descuentoDeDescansoExacto(marcas, dia));
}
function descuentoDeDescansoExacto(marcas, dia) {
    const fijado = minutosDeLaUnion(dia.horaEntrada, leerDescansos(dia.descansos));
    if (fijado <= 0 || !marcas.some(almuerzo_1.seTrabajo))
        return 0;
    const tomados = marcas
        .filter(m => m.salida && m.salidaDescanso)
        .reduce((s, m) => s + (0, almuerzo_1.minutosTomadosEnLaPausa)(m.salida, marcas), 0);
...
function descuentoDeCadaDescanso(marcas, dia) {
    ...
    return ventanas.map(v => {
        const i = asignadas.findIndex(a => a !== null && (0, exports.claveDeVentana)(a) === (0, exports.claveDeVentana)(v));
        const tomados = i < 0 ? 0 : (0, almuerzo_1.minutosTomadosEnLaPausa)(salidas[i].salida, marcas);
        const aqui = Math.min(quedan, Math.max(0, (0, tardanzas_1.duracionFranjaMin)(v.inicio, v.fin) - tomados));
        quedan -= aqui;
        acumulado += aqui;
...
function descansoQueToca(ahora, dia, tomadas) {
    const usadas = new Set(tomadas.map(exports.claveDeVentana));
    const pendientes = ventanasEnOrden(dia.horaEntrada, leerDescansos(dia.descansos))
        .filter(v => !usadas.has((0, exports.claveDeVentana)(v)));
    if (pendientes.length === 0)
        return null;
    const enCurso = pendientes.find(v => (0, almuerzo_1.estaDentroDe)(ahora, { fecha: dia.fecha, inicio: v.inicio, fin: v.fin }));
    if (enCurso)
        return enCurso;
    const entrada = horaValida(dia.horaEntrada);
    const base = dia.fecha.getTime() + (entrada === null ? 0 : (0, tardanzas_1.minutosDe)(entrada) * MS_MIN);
...
function asignarSalidas(dia, salidas) {
    const delDia = new Set(leerDescansos(dia.descansos).map(exports.claveDeVentana));
    const tomadas = [];
    const porSalida = salidas.map(() => null);
    const orden = salidas.map((_, i) => i).sort((a, b) => salidas[a].salida.getTime() - salidas[b].salida.getTime());
    const yaTomada = (v) => tomadas.some(t => (0, exports.claveDeVentana)(t) === (0, exports.claveDeVentana)(v));
    const asignar = (i, v) => {
        if (v)
            tomadas.push(v);
        porSalida[i] = v;
    };
    const porLaGuardada = (i) => {
        const guardada = leerVentana(salidas[i].descansoVentana);
        if (guardada !== null && delDia.has((0, exports.claveDeVentana)(guardada)) && !yaTomada(guardada))
            asignar(i, guardada);
    };
    const porLaHora = (i) => {
        if (porSalida[i] === null)
            asignar(i, descansoQueToca(salidas[i].salida, dia, tomadas));
    };
...
function regresoEsperadoDelDescanso(salida, fecha, v) {
    const ventana = { fecha, inicio: v.inicio, fin: v.fin };
    if ((0, almuerzo_1.estaDentroDe)(salida, ventana))
        return new Date((0, almuerzo_1.finDeLaVentanaDe)(salida, ventana));
    return new Date(salida.getTime() + (0, tardanzas_1.duracionFranjaMin)(v.inicio, v.fin) * MS_MIN);
}
// ─────────────────────────────── EL EDITOR ───────────────────────────────
const minutoDe = (d) => Math.floor(d.getTime() / MS_MIN);
...
function emparejarSalidasDeDescanso(huecos, salidas) {
    const pareja = huecos.map(() => null);
    ...
    const libres = (lista, tomada, lado) => lista
    ...
    for (let tramo = 0; tramo <= anclas.length; tramo++) {
        ...
        for (const [a, b] of enOrdenSinCruzar(h.map(x => x.t), s.map(x => x.t)))
            pareja[h[a].k] = s[b].k;
    }
    return pareja;
}
...
function enOrdenSinCruzar(h, s) {
    if (h.length > s.length)
        return enOrdenSinCruzar(s, h).map(([j, i]) => [i, j]);
    // costo[i][j]: lo mínimo para darles pareja a los primeros i de h entre los primeros j de s.
    const costo = [new Array(s.length + 1).fill(0), ...h.map(() => new Array(s.length + 1).fill(Infinity))];
    ...
    for (let i = h.length; i > 0; i--, j--) {
        ...
        while (j > i && costo[i][j - 1] === costo[i][j])
            j--;
        pares.push([i - 1, j - 1]);
    }
    return pares;
...
function completarVentanasDeDescanso(dia, filas, heredadas) {
    ...
    return filas.map((_, k) => {
        ...
        return v ? (0, exports.claveDeVentana)(v) : null;
    });
}

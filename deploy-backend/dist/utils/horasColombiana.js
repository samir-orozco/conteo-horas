...
function clasificarMinuto(hora, esDomOFestivo, esExtra, horaInicioDiurna, horaFinDiurna) {
    const diurno = esDiurna(hora, horaInicioDiurna, horaFinDiurna);
    if (esDomOFestivo) {
        if (!esExtra)
            return diurno ? 'HDD' : 'HND';
...
function calcularHorasTrabajadas(entrada, salida, festivosDates, tiposHoraDB, jornadaSemanalHoras, minutosOrdinariosSemanaAcumulados = 0) {
    ...
    while (cursor < salida) {
        const zonedCursor = (0, date_fns_tz_1.toZonedTime)(cursor, TZ);
        const hora = zonedCursor.getHours();
        const diaSemana = DIAS[(0, date_fns_1.getDay)(zonedCursor)];
...
function descontarAlmuerzo(resultado, almuerzoMin) {
    if (almuerzoMin <= 0)
        return { descontado: 0 };
    const hod = resultado.find(t => t.codigo === 'HOD');
    if (!hod || hod.minutos <= 0)
        return { descontado: 0 };
    const restar = Math.min(almuerzoMin, hod.minutos);
    hod.minutos -= restar;
    return { descontado: restar };
}
function calcularValorHora(salarioMensual, horasMes) {
